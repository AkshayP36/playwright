...
// test("practice test 001", async({page})=>{
//     await page.goto("http://www.tizag.com/htmlT/htmlcheckboxes.php");
//     // await page.locator('//input[@name="q"]').fill("Where is apple?");
...
//     let result = page.locator('(//div[@class="display"])[1]/input[1]').isChecked();
//     console.log(result);
// });

test("Testing wiki dropdowns", async ({page})=>{
    await page.goto("https://www.wikipedia.org/");
    await page.locator('//select[@id="searchLanguage"]').selectOption('Asturianu');
    await page.waitForTimeout(4000);
    await page.locator('//select[@id="searchLanguage"]').selectOption('Hrvatski');
    await page.waitForTimeout(4000);
    await page.locator('//select[@id="searchLanguage"]').selectOption('Lietuvių');
    await page.waitForTimeout(4000);
});